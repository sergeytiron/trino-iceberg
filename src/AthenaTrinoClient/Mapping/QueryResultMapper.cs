...
public class QueryResultMapper
{
    ...
    /// </summary>
    public List<T> DeserializeResults<T>(IEnumerable<List<object>> rows, IList<TrinoColumn> columns)
    {
    ...
    private static string ConvertSnakeCaseToPascalCase(string snakeCase)
    {
        ...
        return string.Concat(
            parts.Select(p => p.Length > 0 ? char.ToUpperInvariant(p[0]) + p[1..].ToLowerInvariant() : p)
        );
    }
